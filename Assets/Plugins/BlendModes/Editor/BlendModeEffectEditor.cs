...
namespace BlendModes
{
    ...
    public class BlendModeEffectEditor : Editor
    {
        ...
        private static readonly GUIContent shaderFamilyContent = new("Shader Family", "Shader family of the object's render material.");
        private static readonly GUIContent blendModeContent = new("Blend Mode", "Algorithm to be used when blending render layers.");
        private static readonly GUIContent renderModeContent = new("Render Mode", "The way in which blend mode effect is applied to the affected object:\n\n - Self With Screen: Blend object's texture with anything that was drawn before the object on screen. Similar to layer blending modes in Photoshop;\n\n - Texture With Self: Blend specified overlay color and texture with the object's texture. Similar to `Pattern Overlay` layer style in Photoshop.");
        private static readonly GUIContent overlayColorContent = new("Overlay Color", "Color used to blend with the object's texture.");
        private static readonly GUIContent overlayTextureContent = new("Overlay Texture", "Texture used to blend with the object's texture.");
        private static readonly GUIContent overlayTextureOffsetContent = new("Overlay Offset", "UV offset of the overlay texture.");
        private static readonly GUIContent overlayTextureScaleContent = new("Overlay Scale", "UV scale of the overlay texture.");
        private static readonly GUIContent maskModeContent = new("Mask Mode", "The way in which blend mode effect is applied when interacting with the masked areas:\n\n - Disabled: Apply blend effect over the entire object (ignore masks);\n\n - Nothing But Mask: Apply blend effect over the areas affected by mask;\n\n - Everything But Mask: Apply blend effect over the areas not affected by mask.");
        private static readonly GUIContent maskBehaviourContent = new("Mask Behaviour", "What should happen with the pixels affected by the mask:\n\n - Cutout: Pixels over the affected (masked) areas will be discarded;\n\n - Normal: Pixels over the affected (masked) areas will be rendered using default material pass (normal blend mode).");
        private static readonly GUIContent optimizationsContent = new("Optimizations", "Available performance optimizations for the current instance of the effect.");
        private static readonly GUIContent framebufferEnabledContent = new("Framebuffer", "Use `framebuffer_fetch` instruction instead of a grab pass to access the backbuffer. Significantly improves performance, but is only supported on selected devices (most notably PowerVR-based ones on iOS).");
        private static readonly GUIContent unifiedGrabEnabledContent = new("Unified Grab", "Share a grab texture with the other instances, that have this optimization enabled. Significantly improves performance when multiple blend mode effects are used simultaneously, but the instances won't properly blend with each other.");
        private static readonly GUIContent shareMaterialContent = new("Share Material", "Whether to share material instances with the same shader family and blend mode type. Can reduce draw calls count in some cases, but all the material properties will also be shared.");
        private static readonly GUIContent stencilIdContent = new("Stencil ID", "ID of the stencil buffer to use for masking. Change in case default value conflicts with other stencil operations you're performing.");
        ...
        public override void OnInspectorGUI ()
        {
            ...
            if (UrpUtilities.UrpEnabled && !UrpUtilities.FeatureEnabled)
            {
                EnableUrpFeatureGUI();
                return;
            }
        ...
        private void EnableUrpFeatureGUI ()
        {
            EditorGUILayout.HelpBox("URP is enabled, but blend mode effect render feature is not installed.", MessageType.Warning);
            if (GUILayout.Button("Install render feature"))
            {
                #if BLENDMODES_URP
                ...
                if (urp.rendererDataList.Length == 0) Debug.LogError("URP is missing default renderer. Make sure it's correctly configured.");
                var feat = CreateInstance<RenderBlendModeEffect>();
                feat.name = "RenderBlendModeEffect";
                feat.SetActive(true);
                AssetDatabase.AddObjectToAsset(feat, urp.rendererDataList[0]);
                urp.rendererDataList[0].rendererFeatures.Add(feat);
                urp.rendererDataList[0].SetDirty();
        ...
        private void ShaderFamilyGUI ()
        {
            ...
            if (newIndex < 0 || newIndex >= ComponentExtension.SupportedShaderFamilies.Length) newIndex = 0;
        ...
        private void BlendModeGUI ()
        {
            ...
            rect.xMin += EditorGUIUtility.labelWidth + 2;
        ...
        private void RenderModeGUI ()
        {
            ...
            if (!supportsOverlay)
            {
                renderModeProperty.enumValueIndex = (int)RenderMode.SelfWithScreen;
                return;
            }
            if (!supportsGrab)
            {
                renderModeProperty.enumValueIndex = (int)RenderMode.TextureWithSelf;
                OverlayModeGUI();
                return;
            }
